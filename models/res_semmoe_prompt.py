...
class MMoE(nn.Module):
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        res_5 = torch.cat([torch.cat([dtp_output[:,0,:,:,:].unsqueeze(0),res[5]],dim=1),torch.cat([dtp_output[:,1,:,:,:].unsqueeze(0),res[5]],dim=1)],dim=0)
        for i in range(5):
            de_inp = torch.cat([res[i],res[i]],dim=0)
            res[i] = de_inp
        res[5] = res_5
        gates, load = self.noisy_top_k_gating(dtp_output.reshape(size[0], size[1]*size[2]*size[3]*size[4]), self.training)
        # print(gates)
