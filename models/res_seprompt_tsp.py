...
class DTP_COV(nn.Module):
    def __init__(self, in_channels, out_channels, k_size=3, stride=1, padding=1):
        super(DTP_COV, self).__init__()
...
class DTP(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.cov = DTP_COV(in_channels, out_channels)
        self.last_cov = DTP_COV(in_channels, 2)
...
class DecBlock(nn.Module):
    def __init__(self, in_channels, out_channels, mode_upsample=0):
        ...
        if mode_upsample == 0:
            ...
            self.conv1 = ConvBlock(in_channels-1, out_channels)
            ...
            self.conv = ConvBlock(in_channels + out_channels - 1, out_channels)
...
class Decoder(nn.Module):
    def __init__(self, root_feat_maps=16, mode_upsample=0):
        ...
        # from grad_demo import backward_hook
        # self.register_backward_hook(backward_hook)
        self.up5 = DecBlock(root_feat_maps * 32 + 1, root_feat_maps * 16, mode_upsample)
...
class SparseDispatcher(object):
    ...
    def dispatch(self, inp):
        ...
        for i in range(0,self._num_experts):
            input[i] = []
        inp_exp = [inp[i][self._task_index].squeeze(1) for i in range(0,6)]
        #6: encoder的输出是6个tensor
        for i in range(0,6):
            temp = torch.split(inp_exp[i], self._part_sizes_sum, dim=0)
...
class resunet_prompt(nn.Module):
    ...
    def __init__(self, input_size, in_channels, out_channels, num_experts, noisy_gating=True, k=2, task_num=2, attr= [0,0,1]):
        super(resunet_prompt, self).__init__()
        self.dtp_in_channels = 386
        ...
        self.dtp = DTP(self.dtp_in_channels,self.dtp_in_channels)
        self.decoder = nn.ModuleList([Decoder(self.root_feat_maps, mode_upsample=0) for i in range(2)])
        # self.w_gate = nn.Parameter(torch.zeros(512*3*3*3, task_num, num_experts), requires_grad=True)
        self.w_gate = nn.Parameter(torch.zeros(256, task_num, num_experts), requires_grad=True)
        self.w_noise = nn.Parameter(torch.zeros(256, task_num, num_experts), requires_grad=True)
        self.uni_prompt = nn.Parameter(torch.zeros(1,2,2,8,8), requires_grad=True)
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        res = self.encoder(x)
        de_inp = []
        dtp_input = torch.cat([self.uni_prompt,res[5]],dim=1)
        dtp_output = self.dtp(dtp_input)
        size = dtp_output.size()
        organ_decoder = []
        tumor_decoder = []
        res_5 = [torch.cat([dtp_output[:,0,:,:,:].unsqueeze(0),res[5]],dim=1),torch.cat([dtp_output[:,1,:,:,:].unsqueeze(0),res[5]],dim=1)]
        for i in range(5):
            organ_decoder.append(res[i])
            tumor_decoder.append(res[i])
        organ_decoder.append(res_5[0])
        tumor_decoder.append(res_5[1])
        out = [self.decoder[0](organ_decoder),self.decoder[1](tumor_decoder)]
        # out = self.finalconv(out)
        output = [self.finalorganconv(out[0]),self.finaltumorconv(out[1])]
