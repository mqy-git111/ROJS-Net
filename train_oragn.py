...
# import torch
# print(torch.cuda.get_device_name(1))         # 查看使用的设备名称
...
import config
from models import resunet_organ
...
def val(model, val_loader, loss_func, n_labels):
    ...
    with torch.no_grad():
        for idx, (data, targets) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            optimizer.zero_grad()
            output = model(data)
            loss = loss_func(output, labels_organ)
            train_loss.update(loss.item()/2, data.size(0))
            train_organ_dice.update(output, labels_organ)
...
def train(model, train_loader, optimizer, loss_func, n_labels):
    ...
    for idx, (data, targets) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        labels_organ = common.to_one_hot_3d(labels_organ_1, 4).to(device)
        ...
        optimizer.zero_grad()
        output = model(data)
        loss = loss_func(output, labels_organ) + ce(output, labels_organ_1.to(device))
        loss.backward()
        optimizer.step()
    ...
    # sum = sum + train_tumor_dice.avg[1]
    train_log['Train_dice_avg'] = sum / 3
...
if __name__ == '__main__':
    ...
    # dataloader
    save_path = os.path.join('./results/resunet_organ', args.save)
    ...
    model = resunet_organ.UNet(1, args.n_labels, args.task_num, training=True).to(device)
