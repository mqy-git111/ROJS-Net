...
from .dice_loss import DiceLoss_sigmoid,DiceLoss_softmax
...
class SegLoss(nn.Module):
    ...
    def forward(self, predict, gt, flag=1, is_average=True):
        predict = predict.float()
        gt = gt.float()
        if self.activation == 'softmax':
            predict = F.softmax(predict, dim=1)
            if flag != 4:
                predict = predict[:,flag,:,:,:]
                gt = gt[:,flag,:,:,:]
        elif self.activation == 'sigmoid':
            predict = F.sigmoid(predict)
        if flag == 4:
            dice_loss_func = DiceLoss_softmax()
            loss = dice_loss_func(predict, gt, is_average)
        else:
            dice_loss_func = DiceLoss_sigmoid()
            loss = dice_loss_func(predict, gt, is_average)
